...
+ (void)hookWeChat {
    //      微信撤回消息 - (id)filterAddMsg:(id)arg1; //@selector(onRevokeMsg:); @selector(FFToNameFavChatZZ:)
    SEL revokeMsgMethod = LargerOrEqualVersion(@"2.3.22") ? @selector(FFToNameFavChatZZ:sessionMsgList:):@selector(onRevokeMsg:);
    tk_hookMethod(objc_getClass("MessageService"), revokeMsgMethod, [self class], @selector(hook_onRevokeMsg:sessionMsgList:));
    //      微信消息同步
